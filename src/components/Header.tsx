...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-dark-bg border-b border-secondary-foreground/10">
      ...
      {/* Mobile Menu */}
      <div
        className={`lg:hidden absolute top-full left-0 right-0 bg-dark-bg border-b border-secondary-foreground/10 transition-all duration-300 ${
        ...
        <nav className="container mx-auto px-4 py-4 flex flex-col gap-3">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-secondary-foreground/70 hover:text-primary transition-colors duration-300 font-medium py-2"
